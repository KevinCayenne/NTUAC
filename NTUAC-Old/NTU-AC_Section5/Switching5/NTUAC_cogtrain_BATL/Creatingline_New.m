                ...
                for brightness_lv_counter = 1:brightness_lv
                    for angle_lv_counter = 1:angle_lv
                        for thickness_lv_counter = 1:thickness_lv
                            for length_lv_counter = 1:length_lv
...
switch length_cond  % SUperior/INferior
    case 0
        line_COND.location = 'LOng';
        rec_length = ((rec_maxlength - rec_minlength)/5 * 1)/ (length_lv-1) * (length_lv_counter-1) + ...
            (rec_maxlength- ((rec_maxlength - rec_minlength)/5 * 1));
    case 1
        line_COND.location = 'SHort';
        rec_length = ((rec_maxlength - rec_minlength)/5 * 1)/ (length_lv-1) * (length_lv_counter-1) + ...
            rec_minlength;
end

fprintf('%s%0.2f%s%s%0.2f%s%s%0.2f%s%s%0.2f\n', line_COND.brightness(1:2), rec_brightness, ' ,', line_COND.angle(1:2), rec_angle, ' ,',...
    line_COND.thickness(1:2), rec_thickness, ' ,', line_COND.location(1:2), rec_length)
...
pic_name = ['line_' ...
    ...
    line_COND.thickness(1:2) num2str(thickness_lv_counter) '_' ... 
    line_COND.location(1:2) num2str(length_lv_counter) '.jpg'];
